...
x番目とx+n番目のデータを抽出する
"""
class ConsecutiveFramesDataset(torch.utils.data.Dataset):
...
def train_vae(current_time):
    ...
    """
    n = #n番目
    consecutive_frames_dataset = ConsecutiveFramesDataset(dataset, n)
    dataloader = DataLoader(consecutive_frames_dataset, batch_size=BATCH_SIZE_VAE, shuffle=True)
